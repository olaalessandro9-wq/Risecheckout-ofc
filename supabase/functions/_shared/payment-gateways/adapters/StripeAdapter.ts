...
export class StripeAdapter implements IPaymentGateway {
  ...
  async createCreditCard(request: PaymentRequest): Promise<PaymentResponse> {
    try {
      log.info(`Creating card payment for order ${request.order_id}`);
      ...
      if (this.connectedAccountId) {
        ...
        log.info(`Split payment: platform fee ${platformFee} centavos`);
      }
      ...
      log.error(`Error creating card payment:`, error);
      return {
        success: false,
        transaction_id: "",
        status: "error",
  ...
  async createPix(request: PaymentRequest): Promise<PaymentResponse> {
    try {
      ...
      log.info(`PIX Payment Intent created: ${confirmedIntent.id}`);
      ...
      log.error(`Error creating PIX payment:`, error);
      return {
        success: false,
        transaction_id: "",
        status: "error",
  ...
  async createRefund(
    ...
    try {
      log.info(`Creating refund for ${paymentIntentId}`);
