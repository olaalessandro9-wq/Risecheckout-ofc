...
const log = createLogger("buyer-auth-proxy");

// Action mapping from legacy buyer-auth to unified-auth
const ACTION_MAP: Record<string, string> = {
  ...
  // Producer-specific actions (for area de membros)
  "check-producer-buyer": "check-producer-buyer",
  "ensure-producer-access": "ensure-producer-access",
...
serve(async (req) => {
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      // For login/register, inject preferredRole = "buyer"
      if (["login", "register"].includes(unifiedAction)) {
        originalBody.preferredRole = "buyer";
      }
