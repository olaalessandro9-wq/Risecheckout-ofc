 ...
 * Uses a simplified state machine pattern compatible with XState v5.
 * 
 * @version 1.0.0
 */
    ...
    active: {
      on: {
        ...
        LOAD_USERS: {
          actions: assign({
            usersLoading: true,
            users: ({ context }) => ({ ...context.users, error: null }),
          }),
        },
        REFRESH_USERS: {
          actions: assign({
        ...
        USERS_LOADED: {
          actions: assign({
            ...
            users: ({ context, event }) => ({
              ...context.users,
              items: event.data.users,
              emailsMap: event.data.emails,
              error: null,
            }),
          }),
        },
        ...
        LOAD_PRODUCTS: {
          actions: assign({
            ...
            products: ({ context }) => ({ ...context.products, error: null }),
          }),
        },
        REFRESH_PRODUCTS: {
          actions: assign({
        ...
        LOAD_ORDERS: {
          actions: assign({
            ordersLoading: true,
            orders: ({ context }) => ({ ...context.orders, error: null }),
          }),
        },
        REFRESH_ORDERS: {
          actions: assign({
        ...
        LOAD_SECURITY: {
          actions: assign({
            ...
            security: ({ context }) => ({ ...context.security, error: null }),
          }),
        },
        REFRESH_SECURITY: {
          actions: assign({
        ...
        SECURITY_LOADED: {
          actions: assign({
            ...
            security: ({ context, event }) => ({
              ...
              blockedIPs: event.data.blockedIPs,
