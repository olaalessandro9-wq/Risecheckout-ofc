...
export function MetricCard({
  ...
  return (
    ...
    >
      ...
      <div className={`relative bg-card/40 backdrop-blur-xl border border-border/50 rounded-2xl p-6 hover:border-primary/20 transition-all duration-300 hover:shadow-2xl hover:shadow-primary/10 overflow-hidden h-full bg-gradient-to-br ${className || 'from-card/40 to-card/10'}`}>
        ...
        <div className="relative z-10 flex flex-col justify-between h-full">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              {icon && (
                <div className={`p-2.5 rounded-xl ring-1 ring-border/50 group-hover:scale-110 transition-transform duration-300 ${iconClassName || 'bg-primary/10 text-primary'}`}>
                  {icon}
                </div>
              )}
              <span className="text-sm text-muted-foreground font-medium tracking-wide uppercase">{title}</span>
            </div>
            {showEye && (
          ...
          <div className="space-y-2">
            {isLoading ? (
              ...
              <div className="flex items-end justify-between gap-2">
                <p className="text-3xl font-bold text-foreground tracking-tight group-hover:text-primary transition-colors duration-300 relative">
                  {value}
                </p>
                {trend && (
