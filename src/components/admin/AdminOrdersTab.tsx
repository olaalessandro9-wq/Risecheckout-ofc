...
export function AdminOrdersTab({ period }: AdminOrdersTabProps) {
  ...
  const handleViewDetails = useCallback((orderId: string) => {
    ...
    if (order) { setSelectedOrder(order); setIsDialogOpen(true); }
  }, [orders]);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <div className="flex flex-wrap gap-4 pt-4">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <Input placeholder="Buscar por ID, cliente ou produto..." value={searchTerm}
        ...
        <CardContent>
          <OrdersTable orders={transformedOrders} isLoading={isLoading} sortField={sortField}
            sortDirection={sortDirection} onSort={handleSort} onViewDetails={handleViewDetails} />
          {totalPages > 1 && (
            <div className="mt-4">
              <Pagination>
                <PaginationContent>
                  ...
                  {pageNumbers.map((page, index) => (
                    <PaginationItem key={index}>
                      {page === "ellipsis" ? <PaginationEllipsis /> : (
                        <PaginationLink onClick={() => goToPage(page as number)} isActive={currentPage === page}
      ...
      <OrderDetailsDialog open={isDialogOpen} onOpenChange={setIsDialogOpen}
