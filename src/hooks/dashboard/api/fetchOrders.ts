 ...
 * 
 * MIGRATED: Usa API Layer em vez de supabase.from() direto
 * 
...
export async function fetchRecentOrders(
  ...
  const { data, error } = await api.call<{ orders: Order[] }>("dashboard-orders", {
    action: "recent",
    vendorId,
  ...
  if (error) {
    console.error("[fetchRecentOrders] Erro ao buscar pedidos:", error.message);
...
export async function fetchChartOrders(
  ...
  if (error) {
    console.error("[fetchChartOrders] Erro ao buscar pedidos para gráfico:", error.message);
    throw new Error(error.message);
  }
